...
public class GridViewAdapter extends CommonAdapter<String> {
    ...
    public void convert(final ViewHolder helper, final String item) {
        ...
        if (item.equals("camera")) {
            ...
            ImageLoader.getInstance().displayImage(ImageDownloader.Scheme.FILE.wrap(item), mImageView, options);
        }
        // } else {
        // if (item.equals("camera")) {
        // helper.getView(R.id.pic_camera).setVisibility(View.VISIBLE);
        ...
        if (type == 1 || item.equals("camera")) {
            ...
            helper.setImageResource(R.id.pic_item_select, R.drawable.pic_unselected);
        }
        mImageView.setColorFilter(null);
        helper.getView(R.id.pic_camera).setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                SharedPreferences sharedPreferences = mContext.getSharedPreferences("temp", 0);
                // 保存本次截图临时文件名字
                String fileName = ImageTools.getPhotoFileName(ImageTools.PHOTO_NAME);
        ...
        mImageView.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (item.equals("camera")) {// 相机的item调用照相动作
                    ...
                    {
                        if (mSelectedImage.size() >= size) {
                            Toast.makeText(mContext, "图片数量已达到上限", Toast.LENGTH_SHORT)
                                    .show();
                        } else {
